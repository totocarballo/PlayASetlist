...
namespace PlayASetlist
{
    public partial class MainForm : Form
    {
        private readonly string Title = "Play A Setlist";
        private readonly Read TwitchRead = new Read();
        private readonly Read YoutubeRead = new Read();
        ...
        private void ChkTwitch_CheckedChanged(object sender, EventArgs e)
        {
            ...
            {
                if (string.IsNullOrEmpty(TxtTwitchChannel.Text))
                {
                ...
                var correctUser = TwitchRead.SetUser(true, TxtTwitchChannel.Text);

                if (!correctUser)
                {
                    ChkTwitch.Checked = false;
                    MessageBox.Show(this, Phrases.MessageErrorTwitchUser, "Play A Setlist", MessageBoxButtons.OK, MessageBoxIcon.Error);
                    Text = currentTitle;
                    return;
                }

                var status = TwitchRead.Start();

                if (!status)
                {
                    ChkTwitch.Checked = false;
                    MessageBox.Show(this, Phrases.MessageErrorChatReader, "Play A Setlist", MessageBoxButtons.OK, MessageBoxIcon.Error);
                    Text = currentTitle;
                    return;
                }

                TxtTwitchChannel.Enabled = false;
        ...
        private void ChkYoutube_CheckedChanged(object sender, EventArgs e)
        {
            ...
            {
                if (string.IsNullOrEmpty(TxtYoutubeChannel.Text))
                {
                ...
                Text = $"{Title} - Connecting to YouTube chat...";

                var correctUser = YoutubeRead.SetUser(false, TxtYoutubeChannel.Text);

                if (!correctUser)
                {
                    ChkYoutube.Checked = false;
                    MessageBox.Show(this, Phrases.MessageErrorYoutubeUser, "Play A Setlist", MessageBoxButtons.OK, MessageBoxIcon.Error);
                    Text = currentTitle;
                    return;
                }

                var status = YoutubeRead.Start();
                ...
                TxtYoutubeChannel.Enabled = false;

                Text = currentTitle;
            }
            else
            {
                if (YoutubeRead.IsConnected)
                {
                    TxtYoutubeChannel.Enabled = true;
                    YoutubeRead.Stop();
                }
            }
        }
        ...
        private void SettingsSave()
        {
            var settings = new SettingsCurrent
            {
                ...
                YoutubeChannel = TxtYoutubeChannel.Text,
        ...
        private void SetPhrases()
        {
            ...
            TipMessage.SetToolTip(ChkYoutube, Phrases.ToolTipYoutubeChat);
            TipMessage.SetToolTip(TxtYoutubeChannel, Phrases.ToolTipYoutubeChat);
        }
        ...
        private void SettingsStart()
        {
            ...
            TxtYoutubeChannel.Text = SettingsCurrent.YoutubeChannel;
        ...
        private void Vote_ExportSetlist(object sender, EventArgs e)
        {
            ...
            if (YoutubeRead.IsConnected)
            {
                YoutubeRead.ClearCache();
            }

            if (TwitchRead.IsConnected)
            {
                TwitchRead.ClearCache();
            }
